...
class FaceTTS(pl.LightningModule):
    ...
    def compute_loss(self, x, x_lengths, y, y_lengths, spk=None, out_size=None):
        ...
        """
        print(f"spk: {type(spk)}, shape in compute_loss: {spk.shape}")

        x, x_lengths, y, y_lengths = self.relocate_input([x, x_lengths, y, y_lengths])

        spk_img = self.syncnet.forward_vid(spk)
        print(f"spk_img shape: {spk_img.shape}")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        print(f"Batch {batch_idx}: x: {x.shape}, x_len: {x_len.shape}, y: {y.shape}, y_len: {y_len.shape}, face: {face.shape}")
        ...
        return loss
    #if pythorch-lightning version lower than 1.8.6
    #def lr_scheduler_step(self, epoch, lr_scheduler, optimizer):
